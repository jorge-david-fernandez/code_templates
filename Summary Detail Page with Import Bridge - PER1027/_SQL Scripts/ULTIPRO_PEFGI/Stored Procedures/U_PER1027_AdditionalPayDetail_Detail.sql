DROP PROCEDURE IF EXISTS [dbo].[U_PER1027_AdditionalPayDetail_Detail]
GO
SET ANSI_NULLS, QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE dbo.U_PER1027_AdditionalPayDetail_Detail
	@PayDate DATE = NULL,
...
BEGIN
	...
	'WITH ResultSet AS (
		...
		FROM(
		SELECT uapRecID, uapPayDate, uapWeekEndDate, uapDesc, uapHours, uapUnits, uapRate, uapSales, uapProfit, uapSupplemental, uapNotes
		FROM dbo.U_PER1027_AddlPayDetail (NOLOCK)
		JOIN dbo.EmpComp (NOLOCK) ON eecEmpNo = uapEmpNo 
		WHERE CAST(uapPayDate AS DATE)  = ''' + CONVERT(VARCHAR, @PayDate) + ''''
	...
	--exec U_PER1027_AdditionalPayDetail_Detail @FinancialSystem='OPR   ',@OrgLvl='3',@WhereClause=default,@OrderBy=N'uoxUKGValue ',@page_size=20,@page_number=1
END
GO
